<div>
    <x-slot name="meta_title">@lang('app.dashboard') | @lang('app.settings')</x-slot>
    <x-slot name="title">@lang('app.settings')</x-slot>
        ...
        <div class="col-lg-12">
            <div class="card card-primary card-outline">
                <div class="card-header">
                    {{-- <a href="{{ route('admin.setting.create') }}" class="btn btn-primary create-btn"
                                    ...
                                    <tbody>
                                        @forelse ($settings as $setting)
                                            <tr role="row" class="odd">
                                                <td class="table-id">{{ $setting->id }}</td>
                                                <td>{{ $setting->translate('title') }}</td>
                                                <td>
                                                    <div class="actions">

                                                        @if (permation('setting_view'))
                        ...
                        <div class="row mt-4">
                            <div class="col-sm-12 col-md-5">
                                <div class="dataTables_info" id="example1_info" role="status" aria-live="polite">
                                    ...
                                    {{ $settings->count() }}
                                    @lang('app.of') {{ $settings->total() }} @lang('app.entries')</div>
                            </div>
