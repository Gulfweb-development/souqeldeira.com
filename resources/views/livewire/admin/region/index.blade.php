<div>
    <x-slot name="meta_title">@lang('app.dashboard') | @lang('app.regions')</x-slot>
    <x-slot name="title">@lang('app.regions')</x-slot>
    <x-slot name="breadcrumb">
        <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">@lang('app.dashboard')</a></li>
        <li class="breadcrumb-item active">@lang('app.regions')</li>
    </x-slot>
    <div class="row">
            ...
            <div class="card card-primary card-outline">
                <div class="card-header">
                    @if (permationTo('region_create'))
                        <a href="{{ route('admin.region.create') }}" class="btn btn-primary create-btn"
                                    ...
                                    <tbody>
                                        @forelse ($regions as $region)
                                            <tr role="row" class="odd">
                                                <td class="table-id">{{ $region->id }}</td>
                                                <td>{{ $region->translate('name') }}</td>
                                                <td>{{ $region->governorate->translate('name') }}</td>
                                                <td>
                                                    <div class="actions">

                                                        @if (permationTo('region_view'))
                                                            <a href="{{ route('admin.region.show', [$region->id]) }}"
                                                                class="btn bg-gradient-warning btn-sm show-btn"
                                                                title="@lang('app.show')">
                                                                <i class="fas fa-eye"></i>
                                                            </a>
                                                        @endif
                                                        @if (permationTo('region_edit'))
                        ...
                        <div class="row mt-4">
                            <div class="col-sm-12 col-md-5">
                                <div class="dataTables_info" id="example1_info" role="status" aria-live="polite">
                                    ...
                                    {{ $regions->count() }}
                                    @lang('app.of') {{ $regions->total() }} @lang('app.entries')</div>
                            </div>
