...
class BookeeyService {
    ...
    function __construct() {
        ...
        $this->isTestModeEnable = env('PAYMENT_TEST' , false);
        $this->title = TITLE;
        $this->description = DESCRIPTION;
        $this->merchantID = env('PAYMENT_MERCHANT' );
     ...
     */
    function initiatePayment($transactionDetails = null){
        if ( $transactionDetails == null ) {
            $transactionDetails = [ [
                "SubMerchUID" => $this->getMerchantID(),
                "Txn_AMT" => $this->getAmount()
            ] ];
        }
        ...
        $data = "$mid|$orderId|$su|$fu|$amt|$crossCat|$secretKey|$rndnum";
        ...
        if (isset($decodeOutput['PayUrl'])) {
            ...
            throw new Exception("Error Message: ".$decodeOutput['Message']);
        }else{
            throw new Exception('Gateway not response!');
        }
    }
